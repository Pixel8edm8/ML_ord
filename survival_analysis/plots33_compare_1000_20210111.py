...
loss500_ordCOX1 = np.asarray(pd.read_csv("BRCA_methylation_mRNA_without_penalty_term_ml_ordCOX_20210106.csv",header=None))  
...
plt.xlabel('epoch')
plt.legend(['Only main task loss method','Multi-task losses method (ML_ordCOX)','Only auxiliary task loss method'], loc='upper right')
plt.savefig('Loss Curves2_20210111_3curves.jpg', dpi=300) #指定分辨率保存
plt.show()
