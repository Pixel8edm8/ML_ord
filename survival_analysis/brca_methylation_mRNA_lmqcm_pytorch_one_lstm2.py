#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000, c_index=  remove log_var[i] seems as no difference.
"""
...
#############################################################################################################
class Net(nn.Module):
    def __init__(self, input_size, hidden_size, output1_size):
...
#        self.fc1 = nn.Linear(input_size, hidden_size) 
        self.relu = nn.ReLU()
        self.fc11 = nn.Linear(hidden_size, 10)
...
#        out0=self.fc1(x)
        out00 = self.relu(out)
        out000 = self.fc11(out00)
        out = self.Dropout(out000)
        out1 = self.fc2(out)
#        out2 = self.fc3(out)
        return out1#, out2
...
#        inp = Variable(torch.from_numpy(x_train)).type(torch.FloatTensor)
#        target1 = Variable(torch.from_numpy(y_train)).type(torch.FloatTensor)
#        target2 =Variable( torch.from_numpy( ystatus2_train)).type(torch.FloatTensor)
        
        out = model2(inp)
        
        loss = criterion(out, target1)#, [log_var_a, log_var_b])
        ...
        print("epoch=",i+1,":  ", "loss=",loss_history[i], ";  var_a=", log_var_a.item(),";  var_b=",log_var_b.item())
...
#    plt.show() 
    ...
    plt.show()
    # Pred
#    inp_test0 = Variable(torch.from_numpy(x_test)).type(torch.FloatTensor)
    
    inp_test =torch.from_numpy(x_test)# Variable(inp_test).type(torch.FloatTensor)
    
    predicted_main0 = model2(inp_test)#[0]
    
    pred_test = predicted_main0.view(-1).data.numpy()  #转换成一维的ndarray数据，这是预测值
    
    predicted_main=np.sum(predicted_main0.data.numpy(), axis=1)
    
#    predicted_aux0 = model2(inp_test)[1]
#    predicted_aux=np.sum(predicted_aux0.data.numpy(), axis=1)
    
    prediction =predicted_main#+1*predicted_aux
...
np.save("c_indices_main",c_indices_mlp) 
print(c_indices_mlp)
data_a=np.load('c_indices_main.npy')
print(c_index22/10)
aa=0