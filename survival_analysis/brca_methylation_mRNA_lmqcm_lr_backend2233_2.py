...
#    plt.show() 
    ...
    plt.title('Lamda Curves')
    plt.ylabel('train-lamda')
    plt.xlabel('epoch')
    plt.legend(['${lamda0}$','${lamda1}$'], loc='upper right')
    plt.savefig('Lamda Curves.jpg', dpi=300) #指定分辨率保存
    plt.show()
    
    plt.plot(range(EPOCH), hist.history['loss'])
    plt.show()
    np.savetxt("hist.history['loss1000_ml_ordCOX20201220'].csv",hist.history['loss'], delimiter=",")
