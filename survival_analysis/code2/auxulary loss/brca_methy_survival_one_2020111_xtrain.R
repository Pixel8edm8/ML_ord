...
setwd("C:/2019821topc/mRNA/mRNA_Methylstion/auxulary loss")
...
mydata1= read.csv("brca_prediction_leave_one_out_lstm_xtrain_methylation_20200108.csv",header = F, fill = F, stringsAsFactors=FALSE)
month1=read.csv("brca_ytime_test_leave_one_out_lstm_xtrain_methylation_20200108.csv",header = F, fill = F, stringsAsFactors=FALSE)
status1=read.csv("brca_ystatus_test_leave_one_out_lstm_xtrain_methylation_20200108.csv",header = F, fill = F, stringsAsFactors=FALSE)
mydata_train_median= read.csv("brca_ypred_train_median_leave_one_out_lstm_xtrain_methylation_20200108.csv",header = F, fill = F, stringsAsFactors=FALSE)
# 
mydata_train_median=data.matrix(mydata1)
mydata = data.matrix(mydata1)
month=data.matrix(month1)
status=data.matrix(status1)
s1 = nrow(mydata)
s2 = ncol(mydata)
################################################################
mySurv = Surv(month, status);
x = mydata
print(s1)
# leave-one-out CV
group = cbind(numeric(s1))
group1 = cbind(numeric(s1))
mv = median(mydata_train_median)
#mv =mydata_train_median
ind = 1:s1
for(i in 1:s1){
...
png(filename = "C:/2019821topc/mRNA/mRNA_Methylstion/auxulary loss/Survival_one_2020111.png", width = 5.5, height = 5.5,
	units = "cm", res = 300, pointsize = 7)
plot(fit, mark.time=TRUE, xlab = "Months", ylab = "Survival", main=paste("Surival Curve with aux loss ",sep=""), lty = 1:2,
     col = 1:2, cex = 0.5)
grid()
