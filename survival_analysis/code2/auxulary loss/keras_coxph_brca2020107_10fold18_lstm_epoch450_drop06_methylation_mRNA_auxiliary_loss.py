...
#    train_x, test_x, train_y, test_y,= X[train_index], X[val_index], y[train_index], y[val_index] # 这里的X_train，y_train为第iFold个fold的训练集，X_val，y_val为validation set
    x_train, x_test, y_train, y_test, ytime_train, ytime_test, ystatus_train, ystatus_test,data_methylation_train,data_methylation_test =\
        ...
        data_methylation[train_index],data_methylation[val_index]
...
#    ytrue_dim=y_train.shape[1]
    input_dim_clinic =data_methylation_train.shape[2]
...
#    out1=Bidirectional(LSTM(55,activation='linear',return_sequences=True,kernel_initializer=glorot_uniform(),kernel_regularizer=l2(reg),activity_regularizer=l2(0.001)), merge_mode='concat')(title_input)
    out_gene=Bidirectional(LSTM(100,activation='linear',return_sequences=False), merge_mode='concat')(gene_input)
...
#    GRU(100,  activation='linear', return_sequences=True)(title_input)
     # clinic Only
    clinic_input = Input(name='clinic_input', shape=(1,input_dim_clinic))
    
    out_clinic=Bidirectional(LSTM(100,activation='linear',return_sequences=False), merge_mode='concat')(clinic_input )
#    out21=TimeDistributed(Dense(20, activation='tanh'))(out11)
#    out_clinic=Bidirectional(LSTM(10))(out21)
    
    # combined with GRU output
    com = Concatenate(axis=1)([out_gene, out_clinic])
   
     
    out222=Dense(20, activation='tanh')(com)

    
#    GRU(50, activation='tanh', return_sequences=False)(out1)
    out3=Dropout(0.1)(out222)
    main_output= Dense(1,activation='linear',name='main_output')(out3)
#    main_output=main_output0[:,-1,:]
...
#    model=Model(inp, output)
    ...
    hist=model.fit({'gene_input':x_train,'clinic_input':data_methylation_train}, {'main_output': y_train, 'aux_output': y_train}, batch_size = n1, epochs =450)
