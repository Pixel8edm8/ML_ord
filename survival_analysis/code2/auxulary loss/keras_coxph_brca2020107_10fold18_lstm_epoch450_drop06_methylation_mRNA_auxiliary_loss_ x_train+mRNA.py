#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# x_train+mRNA lamda=5, epoch=450，c_index_lstm=0.6044057613802665
"""
...
#    GRU(100,  activation='linear', return_sequences=True)(title_input)
     # clinic Only
    clinic_input = Input(name='clinic_input', shape=(1,input_dim_mRNA))
...
#    model=Model(inp, output)
    ...
    hist=model.fit({'gene_input':x_train,'clinic_input':data_mRNA_train}, {'main_output': y_train, 'aux_output': y_train}, batch_size = n1, epochs =450)
    
    (predicted_main, predicted_aux)=model.predict({'gene_input': x_test,'clinic_input':data_mRNA_test},verbose=1)
