#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# x_train+10*methylation , epoch=500，c_index_lstm=？
"""
...
#    model=Model(inp, output)
    ...
    hist=model.fit({'gene_input':x_train,'clinic_input':data_methylation_train}, {'main_output': y_train, 'aux_output': y_train}, batch_size = n1, epochs =420)
...
#    prediction_train_median = sum(model.predict(x_train))/n1
    ...
    (prediction_train_median, predicted_aux2)=model.predict({'gene_input': x_train,'clinic_input':data_methylation_train},verbose=1)
#    
    prediction_train_median2 = np.sum(prediction_train_median)/n1
    ypred_train.append(prediction_train_median2)
...
#    del model, prediction,  x_train, x_test, y_train, y_test, ytime_train, ytime_test, ystatus_train, ystatus_test,data_methylation_train,data_methylation_test,data_mRNA_train,data_mRNA_test
#    gc.collect()
    K.clear_session()
    tf.reset_default_graph()
np.savetxt("brca_prediction_leave_one_out_lstm_xtrain_methylation_20200108.csv", ypred, delimiter=",")
np.savetxt("brca_ytime_test_leave_one_out_lstm_xtrain_methylation_20200108.csv", time_new, delimiter=",")
np.savetxt("brca_ystatus_test_leave_one_out_lstm_xtrain_methylation_20200108.csv", status_new, delimiter=",")
np.savetxt("brca_ypred_train_median_leave_one_out_lstm_xtrain_methylation_20200108.csv", ypred_train, delimiter=",")
...
#plt.show()
#model.get_weights()
df = pd.read_csv("brca_prediction_leave_one_out_lstm_xtrain_methylation_20200108.csv",header=None)    
month=np.asarray(pd.read_csv("brca_ytime_test_leave_one_out_lstm_xtrain_methylation_20200108.csv",header=None)) 
status=np.asarray(pd.read_csv("brca_ystatus_test_leave_one_out_lstm_xtrain_methylation_20200108.csv",header=None)) 
...
np.save("c_indices_nn_leave_one_out_lstm_xtrain_methylation_20200108.csv",c_indices_lstm) 
print(c_indices_lstm)
data_a=np.load('c_indices_nn_leave_one_out_lstm_xtrain_methylation_20200108.csv.npy')
aa=0