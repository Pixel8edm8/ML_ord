#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#c-index= 0.6486174227836401 leave one out good
"""
...
x = kidtx.drop(["Unnamed: 0","methylation...17.","V1", "erged_data33"], axis = 1).values
...
#age=np.transpose(np.array((kidtx["erged_data6_Age"])))ged_data33=status
sample=y.shape[0]
x = np.asarray(x)
...
# Build model structure
    ...
    model = Sequential()
    model.add(Dense(units = 50, activation = "tanh", use_bias = False, input_shape=[38]))
    model.add(Dense(1, kernel_initializer='normal'))
    model.summary()
#    model.add(Dense(10)) 
...
# partial likelihood depends on all observations
    hist=model.fit(x_train, y_train, batch_size = n1, epochs = 500)
    hist.history['loss']
...
np.savetxt("brca_prediction828_121.csv", ypred, delimiter=",")
np.savetxt("brca_ytime_test828_121.csv", time_new, delimiter=",")
np.savetxt("brca_ystatus_test828_112.csv", status_new, delimiter=",")
np.savetxt("brca_ypred_train_median828_121.csv", ypred_train, delimiter=",")
