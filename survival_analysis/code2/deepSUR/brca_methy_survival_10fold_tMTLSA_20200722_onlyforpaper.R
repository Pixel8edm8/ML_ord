...
setwd("C:/IJCAI2020/code/deepSUR")
...
mydata1= read.csv("brca_prediction828_12.csv",header = F, fill = F, stringsAsFactors=FALSE)
month1=read.csv("brca_ytime_test828_12.csv",header = F, fill = F, stringsAsFactors=FALSE)
status1=read.csv("brca_ystatus_test828_12.csv",header = F, fill = F, stringsAsFactors=FALSE)
#mydata_train_median= read.csv("brca_pred778_779_828_12.csv",header = F, fill = F, stringsAsFactors=FALSE)
# 
#mydata_train_median=data.matrix(mydata_train_median)
#mydata00 =rbind( mydata1,mydata1[2:100,])
#mydata =mydata00[2:486,]
...
mydata00 =rbind( mydata1,mydata1)
#mydata00 =data.matrix(rbind( mydata,data.matrix(mydata[2:100,1])))
#
...
n2 = sum(group==2)
leg2 = paste("High risk(", n2+9, ")", sep = "")

png(filename = "C:/IJCAI2020/code/deepSUR/Survival_10fold_MTLSA_onlyforpaper_20200722.png", width = 5.5, height = 5.5,
	units = "cm", res = 300, pointsize = 7)
plot(fit, mark.time=TRUE, xlab = "Months", ylab = "Survival", main=paste("MTLSA ",sep=""), lty = 1:2,
     col = 1:2, cex = 0.5)
grid()
