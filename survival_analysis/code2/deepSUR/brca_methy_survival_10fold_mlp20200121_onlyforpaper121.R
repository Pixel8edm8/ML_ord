...
mydata1= read.csv("brca_prediction828_121.csv",header = F, fill = F, stringsAsFactors=FALSE)
month1=read.csv("brca_ytime_test828_121.csv",header = F, fill = F, stringsAsFactors=FALSE)
status1=read.csv("brca_ystatus_test828_121.csv",header = F, fill = F, stringsAsFactors=FALSE)
#mydata_train_median= read.csv("brca_pred778_779_828_121.csv",header = F, fill = F, stringsAsFactors=FALSE)
# 
...
png(filename = "C:/IJCAI2020/code/deepSUR/Survival_10fold_DeepSurv_onlyforpaper121.png", width = 5.5, height = 5.5,
	units = "cm", res = 300, pointsize = 7)
plot(fit, mark.time=TRUE, xlab = "Months", ylab = "Survival", main=paste("MLP ",sep=""), lty = 1:2,
     col = 1:2, cex = 0.5)
grid()
