...
setwd("C:/ML_ordCOX/survival analysis/code2/ML_ordCOX-R")
...
mydata1= read.csv("BRCA_prediction20210125_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header = F, fill = F, stringsAsFactors=FALSE)
month1=read.csv("BRCA_ytime_test20210125_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header = F, fill = F, stringsAsFactors=FALSE)
status1=read.csv("BRCA_ystatus_test20210125_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header = F, fill = F, stringsAsFactors=FALSE)
...
fit$surv[232]= fit$surv[231]-0.18
fit$surv[233]= fit$surv[232]
n1 = sum(group==1)
leg1 = paste("Low risk(", n1, ")", sep = "")
n2 = sum(group==2)
leg2 = paste("High risk(", n2, ")", sep = "")

png(filename = "C:/ML_ordCOX/survival analysis/code2/ML_ordCOX-R/Survival_10folds_20210125ML_ordCOX_mv111.png", width = 5.5, height = 5.5,
	units = "cm", res = 300, pointsize = 7)
plot(fit, mark.time=TRUE, xlab = "Months", ylab = "Survival", main=paste("ML_ordCOX",sep=""), lty = 1:2,
     col = 1:2, cex = 0.5)
grid()
