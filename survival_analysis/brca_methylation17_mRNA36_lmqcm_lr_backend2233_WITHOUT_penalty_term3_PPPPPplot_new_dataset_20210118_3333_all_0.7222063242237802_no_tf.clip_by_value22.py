...
model can be used in a NN loss function. An example shows how a NN Without
...
# y_true = (n x 2) array Without y_true[i, 0] the survival time
...
#    Without g.gradient_override_map({"PyFunc": grad_name}):
...
#    print('a_sorted:', a_sorted)
    ...
    # Difference a[n+1] - a[n] of non zero indexes (Gives index ranges of patients Without same timesteps)
...
#    plt.show() 
    ...
    plt.title('Weight Curves Without Penalty Term')
    plt.ylabel('Train-weight Value')
    plt.xlabel('epoch')
    plt.legend(['Weight K1 for Main Task','Weight K2 for Auxiliary Task'], loc='center right')
    plt.savefig('Weight Curves Without penalty term.jpg', dpi=300) #指定分辨率保存
    plt.show()
...
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    plt.savefig('Loss Curves Without Penalty Term.jpg', dpi=300) #指定分辨率保存
    plt.show()
...
print("BRCA_methylation17_mRNA36_lmqcm_lr_backend2233_Without_penalty_term3_no_plot_new_dataset_20201230_3.py") 
print("BRCA:dataX=data_methylation:",c_indices_mlp)
