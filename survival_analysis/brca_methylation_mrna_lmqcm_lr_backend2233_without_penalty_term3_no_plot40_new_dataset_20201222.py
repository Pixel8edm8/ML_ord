#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000, c_index=0.6937330090141651 ？ remove log_var[i] seems as no difference.
"""
...
kidtx = pd.read_csv('brca_Surv_data_methylation_mRNA_all_lmqcm20201222_0.csv')
...
data_methylation=dataX[:,0:33]
data_mRNA=dataX[:,33:n0]
...
#    dense1_layer_model = Model(inputs=model.input, outputs=model.get_layer('main_output').output)
    ...
    Lambdas = varHistory() 
    hist = model.fit([data_mRNA_train,data_methylation_train, y_train,   y_train], batch_size = n1, epochs =EPOCH,callbacks=[reduce_lr,Lambdas])
...
#    pylab.plot(hist.history['loss'])
    ...
    predicted_main, predicted_aux = dense1_layer_model.predict([data_mRNA_test,data_methylation_test,y_test, y_test],verbose=1)
