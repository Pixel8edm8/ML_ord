#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000, c_index=0.6938045500071541 remove log_var[i] seems as no difference.
"""
...
#'''
    class varHistory(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, batch, logs={}):
            self.lamdas0.append( K.get_value(self.model.layers[-1].log_vars[0]))
            self.lamdas1.append( K.get_value(self.model.layers[-1].log_vars[1]))
 #############################################################################################################################    
...
#    dense1_layer_model = Model(inputs=model.input, outputs=model.get_layer('main_output').output)
    ...
    Lamdas = varHistory() 
    hist = model.fit([x_train,x_train, y_train,  ystatus2_train], batch_size = n1, epochs =EPOCH,callbacks=[reduce_lr,Lamdas])
    
    lamda0=np.asarray( Lamdas.lamdas0)
    lamda1=np.asarray( Lamdas.lamdas1)
    
    std0=(np.exp(-lamda0))**0.5
    std1=(np.exp(-lamda1))**0.5
    
    plt.plot(range(EPOCH), std0)
#    plt.show() 
    plt.plot(range(EPOCH), std1)
    plt.title('Lamda Curves without penalty term')
    plt.ylabel('Train-lamda Value')
    plt.xlabel('epoch')
    plt.legend(['${lamda0}$','${lamda1}$'], loc='upper right')
    plt.savefig('Lamda Curves without penalty term.jpg', dpi=300) #指定分辨率保存
    plt.show()
