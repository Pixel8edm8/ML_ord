#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000,lr=0.0005 c_index=0.702389469165832 无lamda1*lamda2
"""
...
#            learning_rate=learning_rate*0.5
        ...
        for j in range(n1//batch_size):
        
            optimizer.zero_grad()
            if j<n1//batch_size-1:
                inp = torch.from_numpy(x_train[(j*batch_size):((j+1)*batch_size)])
                target1 = torch.from_numpy(y_train[(j*batch_size):((j+1)*batch_size)])
                target2 = torch.from_numpy( ystatus2_train[(j*batch_size):((j+1)*batch_size)])
            else:
                inp = torch.from_numpy(x_train[(j*batch_size):n1])
                target1 = torch.from_numpy(y_train[(j*batch_size):n1])
                target2 = torch.from_numpy( ystatus2_train[(j*batch_size):n1])  
            jj=j
            out = model2(inp)
...
#        optimizer.step()
        ...
        loss_history[i] = epoch_loss* batch_size / n1
