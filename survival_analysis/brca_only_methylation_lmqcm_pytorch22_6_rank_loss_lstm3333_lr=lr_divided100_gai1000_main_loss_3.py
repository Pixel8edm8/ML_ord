#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000,lr=0.0005 lr/200 c_index=0.5552251359226217  无lamda1*lamda2
"""
...
##############################
# load dataset
kidtx = pd.read_csv('eigengene_matrix_methylation.csv')
x =kidtx.drop(["Unnamed: 0","V2","V3"], axis = 1).values
y = np.transpose(np.array((kidtx["V2"], kidtx["V3"]))) # V1=time; erged_data33=status
...
ytime=np.transpose(np.array(kidtx["V2"])) # only V1=time;
ystatus= np.transpose(np.array(kidtx["V3"])) #only erged_data33=status
...
#1     5      0   51           0           1             0
#2     7      1   55           0           1             0
#x = kidtx.drop(["Egrey", "V1", "erged_data33"], axis = 1).values
...
#######################################################################################################################################
def neg_log_pl_torch0(y_true, y_pred):
   
    time=y_true[:, 0].data.numpy()
    event = y_true[:, 1].data.numpy()
    xbeta= y_pred.data.numpy()
    xbeta=np.sum(xbeta,axis=1)
    risk = np.exp( xbeta)
    denom =np.cumsum(risk)
    
    terms =  xbeta - np.log(denom)
    loglik = -(event * terms)   #cast将x的数据格式转化成dtype
    return torch.from_numpy(loglik)
    
    
#    lifetime=y_true[:, 0].data.numpy()
#    censor = y_true[:, 1].data.numpy()
#    t, H = unique_set(lifetime)
#    keep_index = np.nonzero(censor)[0]  #censor = 1
#    H = [list(set(h)&set(keep_index)) for h in H]
#    n = [len(h) for h in H]
#    
#    score1 = y_pred.data.numpy()
#    total = 0
...
#######################################################################################################################################
def neg_log_pl_torch(y_true, y_pred):
   
    time=y_true[:, 0].data.numpy()
    event = y_true[:, 1]#.data.numpy()
    xbeta= y_pred#.data.numpy()
    xbeta=torch.sum(xbeta,1)
    risk = torch.exp( xbeta)
    denom =torch.cumsum(risk,0)
    
    terms =  xbeta - torch.log(denom)
    loglik = -(event * terms)  
    
    return torch.sum(loglik ) #torch.from_numpy(loglik)
###################################################################################################################################################33    
def neg_log_pl_torch2 (y_true, y_pred):
  E=  y_true[:, 1] 
...
###################################################################################################################################################
def ordinal_loss2 (y_true, y_pred):
    time=y_true[:, 0].data.numpy()
    event = y_true[:, 1].data.numpy()
    xbeta= y_pred#.data.numpy()
    matrix_risk=np.zeros([time.shape[0]],dtype=np.float32)
    matrix_risk= torch.from_numpy(matrix_risk)
    
    one=np.zeros([1],dtype=np.float32)
    one= torch.from_numpy(one)
###########
    t0, H0 = unique_set(time) # t:unique time. H original index.
    
    actual_event_index = np.nonzero(event)[0]
...
#        m=kk_n[j]
#        Hj=sum(H[j:],[])
        matrix_j = np.zeros([time.shape[0]])
        matrix_j = matrix_j.astype('float32')
        matrix_j= torch.from_numpy(matrix_j)
...
#                print('ii:',ii)
                ...
                rec= one-torch.exp(risk_j-risk_more_j)
#                rec=torch.max(0.,rec)
            
                shape = time.shape[0]
#                one_hot_j = tf.one_hot(Hj[ii],shape[0],dtype=tf.float32)
                one1= torch.FloatTensor(shape,).zero_()
                one_hot_more_j =one1.scatter_(dim=0,index=torch.LongTensor([Hj[ii]]),value=1)
...
#                new_tensor = matrix_risk+risk_j * one_hot_j
        ...
        #        tf.reduce_sum(tf.one_hot(sum(H[13:],[]),n1,dtype=tf.float32),axis=0)
        matrix_risk= matrix_risk+ matrix_j 
    cost2 = -torch.sum(matrix_risk)/(len(Hj))
#    print("cost2:",cost2)
    return cost2 
...
#          precision=1    
#      if i==0:
#          lossA=neg_log_pl_torch(y_true[i], y_pred[i])
#          print("Neg_loss:",lossA.item())
#      if i==1:
#          lossA=ordinal_loss2(y_true[i], y_pred[i])
#          print("Ordinal_loss:",lossA.item())
##      loss += torch.sum(precision * lossA + log_vars[i], -1)
#      
#      loss += precision * lossA 
   lossA=neg_log_pl_torch(y_true, y_pred)  
   loss =torch.sum(lossA , -1)
...
#        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,num_layers=2,dropout=0.2,bidirectional=False)
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,num_layers=1,batch_first=True)                                                          
        self.fc1 = nn.Linear(hidden_size, 10) 
#        self.relu = nn.ReLU()
#        self.fc11 = nn.Linear(20, 10)
        self.Dropout = nn.Dropout(p=0.1)
#        self.fc3 = nn.Linear(hidden_size*2, output2_size)
        self.fc2 = nn.Linear(10, output1_size)
        self.fc3 = nn.Linear(10, output2_size)
    
    def forward(self, x):
#        out0, (_, _) = self.lstm(x)
        out0, (hn, cn) = self.lstm(x)
        out0_1=out0[:,-1,:]
        out0_0=hn[-1,:,:]
        out0_00=self.fc1(out0_1)
#        out00 = self.relu(out0_00)
#        out000 = self.fc11(out0_00)
        out = self.Dropout(out0_00)
        out1 = self.fc2(out)
#        out2 = self.fc3(out)
        return out1#, out2
...
#                                bidirectional=True)    
#    out_gene22= nn.LSTM(input_size=input_dim, hidden_size=100,dropout=0.1, bidirectional=True)
#    out_clinic22=nn.LSTM(input_size=input_dim, hidden_size=100, bidirectional=True)
    
    model2=Net(input_dim, 100, 1, 1)
    
#    out_clinic2=Net(input_dim, 100, 1, 1)
...
#    params = ([p for p in model2.parameters()] + [log_var_a] + [log_var_b])
    params = [p for p in model2.parameters()]
    #optimizer = optim.SGD(params, lr=0.001, momentum=0.9)
    learning_rate=0.001
    lr2=[0.001,0.0005,0.0004,0.0002,0.0001,0.00005,0.00004,0.00002,0.00001,0.000005,0.000002]
    optimizer = optim.Adam(params,lr=0.001)
    
    loss_history = np.zeros(nb_epoch)
    
    loss_history2 = np.zeros(nb_epoch)
#    loss_history2=np.zeros([nb_epoch],dtype=np.float32)
#    loss_history2= torch.from_numpy(loss_history2)
############# convert data into torch from numpy array
    x_train = x_train.astype('float32')
    y_train = y_train.astype('float32')
    ystatus2_train = ystatus2_train.astype('float32')
    
    x_test = x_test.astype('float32')
    y_test = y_test.astype('float32')
    ystatus2_test = ystatus2_test.astype('float32')
    std01=[]
    std02=[]
...
#            learning_rate=learning_rate*0.5
        ...
        target1 = torch.from_numpy(y_train)
        target2 = torch.from_numpy( ystatus2_train)
        
        out = model2(inp)
        
#        loss = criterion(out, [target1, target1], [log_var_a, log_var_b])
        ...
        print("epoch=",i+1,":  ", "loss=",loss_history[i], ";  var_a=", log_var_a.item(),";  var_b=",log_var_b.item(),"LR=", learning_rate)
        print("epoch=",i+1,"_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _")
        std_1 = (torch.exp(-log_var_a)**0.5).item()
        std_2 = (torch.exp(-log_var_b)**0.5).item()
#        std01.append(log_var_a.item())
#        std02.append(log_var_b.item())
        std01.append(std_1)
        std02.append(std_2)
    # plot loss history

   #    pylab.plot(loss_history)
    plt.plot(range(i+1), std01)
#    plt.show() 
    plt.plot(range(i+1), std02)
    plt.show()
    
    plt.plot(range(i+1), loss_history)
    plt.show()
    
    np.savetxt("hist.history['loss1000_Mythlation'].csv", loss_history, delimiter=",")
    # Pred
    inp_test = torch.from_numpy(x_test)
    predicted_main = model2(inp_test)#[0]
    predicted_main=np.sum(predicted_main.data.numpy(), axis=1)
    
#    predicted_aux = model2(inp_test)[1]
#    predicted_aux=np.sum(predicted_aux.data.numpy(), axis=1)
    
    prediction =1*predicted_main#+0*predicted_aux
...
np.savetxt("brca_prediction1204_18lstm2222_epoch400_drop01_main.csv", ypred, delimiter=",")
np.savetxt("brca_ytime_test1204_18lstm2222_epoch400_drop01_main.csv", time_new, delimiter=",")
np.savetxt("brca_ystatus_test1204_18lstm2222_epoch400_drop01_main.csv", status_new, delimiter=",")
np.savetxt("brca_ypred_train_median1204_18lstm2222_epoch400_drop01_main.csv", ypred_train, delimiter=",")

df = pd.read_csv("brca_prediction1204_18lstm2222_epoch400_drop01_main.csv",header=None)    
month=np.asarray(pd.read_csv("brca_ytime_test1204_18lstm2222_epoch400_drop01_main.csv",header=None)) 
status=np.asarray(pd.read_csv("brca_ystatus_test1204_18lstm2222_epoch400_drop01_main.csv",header=None)) 
