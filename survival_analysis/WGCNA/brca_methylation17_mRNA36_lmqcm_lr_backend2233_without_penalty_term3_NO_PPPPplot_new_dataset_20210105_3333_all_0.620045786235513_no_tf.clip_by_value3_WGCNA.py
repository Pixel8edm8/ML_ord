...
kidtx1 = pd.read_csv('methylation_mRNA_merge_WGCNA_20210107.csv')
#kidtx2 = pd.read_csv('brca_Surv_data_methylation_important_p001.csv')
#kidtx3 = pd.read_csv('brca_Surv_data_mRNA_important_p001.csv')

#dataX1 =kidtx1.drop(["fpkm2...1.","Unnamed: 0","Luminal","V1", "erged_data33","MEgrey.x","MEgrey.y"], axis = 1).values
dataX1 =kidtx1.drop(["fpkm2...1.","Unnamed: 0","Luminal","V1", "erged_data33"], axis = 1).values
y = np.transpose(np.array((kidtx1["V1"], kidtx1["erged_data33"]))) # V1=time; erged_data33=status
...
ytime=np.transpose(np.array(kidtx1["V1"])) # only V1=time;
ystatus= np.transpose(np.array(kidtx1["erged_data33"])) #only erged_data33=status
...
#    plt.savefig('Lambda Curves without penalty term3_methylation.jpg', dpi=300) #指定分辨率保存
#    plt.show()
#    
...
#    plt.savefig('Loss Curves without penalty term3_methylation.jpg', dpi=300) #指定分辨率保存
#    plt.show()
#    
#    
#    np.savetxt("BRCA_methylation_mRNA_without_penalty_term_ml_ordCOX_20210106_methylation.csv",hist.history['loss'], delimiter=",")
    
#    import pylab
#    pylab.plot(hist.history['loss'])
#    print([np.exp(-K.get_value(log_var[0]))**0.5 for log_var in model.layers[-1].log_vars])
    print('lambda1 for Main Loss=',std0[999])
    print('lambda2 for Auxiliary Loss=',std1[999])
...
np.savetxt("BRCA_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv", ypred, delimiter=",")
np.savetxt("BRCA_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv", time_new, delimiter=",")
np.savetxt("BRCA_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv", status_new, delimiter=",")
np.savetxt("BRCA_ypred_train_median1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv", ypred_train, delimiter=",")

df = pd.read_csv("BRCA_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header=None)    
month=np.asarray(pd.read_csv("BRCA_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header=None)) 
status=np.asarray(pd.read_csv("BRCA_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term.csv",header=None)) 
...
print("BRCA_methylation17_mRNA36_lmqcm_lr_backend2233_without_penalty_term3_no_plot_new_dataset_WGCNA.py") 
#print("BRCA:dataX=data_all:",c_indices_mlp)
#print("BRCA:dataX=data_mRNA:",c_indices_mlp)
print("BRCA:dataX=data_mRNA+methylation:",c_indices_mlp)
