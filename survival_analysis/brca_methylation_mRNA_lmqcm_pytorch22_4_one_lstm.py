#!/usr/bin/env python3
# -*- coding: utf-8 -*-
#NO RISK,epochs =1000, c_index=0.6868650736872228 remove log_var[i] seems as no difference.
"""
...
h_state = None
class Net(nn.Module):
    def __init__(self, input_size, hidden_size,num_layers, output1_size):
        super(Net, self).__init__()
#        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,num_layers=2,bidirectional=True)
        self.lstm = nn.RNN(input_size=input_size, hidden_size=hidden_size,num_layers=1,batch_first=True) 
...
#        out0, (_, _) = self.lstm(x)
        out0, h_state = self.lstm(x,h_state)
        out=out0[:,-1,:]
#        out0=self.fc1(x)
        out00 = self.relu(out)
...
#    out_clinic22=nn.LSTM(input_size=input_dim, hidden_size=100, bidirectional=True)
    input_size =input_dim                                  #定义超参数输入层，输入数据为1维
    output_size = 1                                 #定义超参数输出层，输出数据为1维
    num_layers = 1                                  #定义超参数rnn的层数，层数为1层
    hidden_size = 50                                #定义超参数rnn的循环神经元个数，个数为32个
    learning_rate = 0.01                            #定义超参数学习率
    train_step = 1                                #定义训练的批次，3000个数据共训练1000次，
    time_step = 3                                  #定义每次训练的样本个数每次传入3个样本
#    h_state = None                                  #初始化隐藏层状态
    
    model2=Net(input_size, hidden_size, num_layers, output_size)
...
#        out = model2(inp)
        out, h_state =model2.forward(inp, h_state)
