...
###################################################################################################################################################
def ordinal_loss2 (y_true, y_pred,t0,H0, Hj):
    time=y_true[:, 0].data.numpy()
    event = y_true[:, 1].data.numpy()
    xbeta= y_pred#.data.numpy()
...
#                print('ii:',ii)
                ...
                rec= 1-torch.exp(risk_j-risk_more_j)
#                rec2=tf.maximum(0.,rec)
...
###################   #######################################################################
def criterion(y_pred, y_true, log_vars,t0,H0, Hj):
   loss = 0
   for i in range(len(y_pred)):
...
#          precision=1    
      ...
      if i==1:
          lossA=ordinal_loss2(y_true[i], y_pred[i],t0,H0, Hj)
...
############# convert data into torch from numpy array
    ...
    for i in range(nb_epoch):
        ...
        loss = criterion(out, [target1, target1], [log_var_a, log_var_b],t0,H0,Hj)
