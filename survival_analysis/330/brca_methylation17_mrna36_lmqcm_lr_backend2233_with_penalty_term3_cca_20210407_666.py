...
def scca_loss_u(x_proj,y_proj,u,v):
#   
    x1tx2 = tf.abs(tf.matmul(tf.transpose(x_proj), y_proj))
    ux1tx2 = tf.matmul(tf.transpose(u), x1tx2)
    ux1tx2v =tf.matmul(ux1tx2, v)
...
def scca_loss_v(x_proj,y_proj,u,v):
#   
    x1tx2 = tf.abs(tf.matmul(tf.transpose(x_proj), y_proj))
    ux1tx2 = tf.matmul(tf.transpose(u), x1tx2)
    ux1tx2v =tf.matmul(ux1tx2, v)
...
#        v = K.get_variable('v', shape=(ys_pred.shape[-1], 1))
        ...
        for i in range(self.nb_outputs+1):
...
#                 lossA.set_shape((ys_true[i].shape[0],))
#                lossA=LOSS_L2(ys_true[i], ys_pred[i])
                lossA=scca_loss_u(ys_true, ys_pred,uu,vv)
                loss += K.sum(precision * lossA+ log_var[i], -1)
...
#                 lossA.set_shape((ys_true[i].shape[0],))
#                lossA=LOSS_L2(ys_true[i], ys_pred[i])
                lossA=scca_loss_v(ys_true, ys_pred,uu,vv)
                loss += K.sum(precision * lossA+ log_var[i], -1)    
#                lossA=neg_log_pl(ys_true[i-1], ys_pred[i])
#                print("Ordinal_loss:",lossA.item())
#                lossA=tf.py_function(func=ordinal_loss, inp=[ys_true[i], ys_pred[i]], Tout=tf.float32) 
...
#    train_x, test_x, train_y, test_y,= X[train_index], X[val_index], y[train_index], y[val_index] # 这里的X_train，y_train为第iFold个fold的训练集，X_val，y_val为validation set
    ...
    #######################################################################################
        class varHistory2(keras.callbacks.Callback):
            def on_train_begin(self, logs={}):
                self.lambdas0 = []
                self.lambdas1 = []
                self.u = []
    ...
    #    dense1_layer_model.summary()
        ...
        reduce_lr2 = LearningRateScheduler(scheduler2)
        Lambdas2 = varHistory2() 
        hist = model.fit([data_mRNA_train2,data_methylation_train2], batch_size = n1, epochs =EPOCH2,callbacks=[reduce_lr2,Lambdas2])
        
        
        lambda0=np.asarray( Lambdas2.lambdas0)
        lambda1=np.asarray( Lambdas2.lambdas1)
        uu=np.asarray( Lambdas2.uu)
        vv=np.asarray( Lambdas2.vv)
        
        std0=(np.exp(-lambda0))#**0.5
        std1=(np.exp(-lambda1))#**0.5
        print("iFold2=",iFold2)
#        plt.plot(range(EPOCH2), lambda0,color='red')
#    #    plt.show() 
#        plt.plot(range(EPOCH2), lambda1,color='purple')
...
#        plt.savefig('Lambda Curves315.jpg', dpi=300) #指定分辨率保存
#        plt.show()
#        
#        plt.plot(range(EPOCH2), std0,color='red')
#    #    plt.show() 
#        plt.plot(range(EPOCH2), std1,color='purple')
...
#        plt.savefig('Weight Curves without penalty term315.jpg', dpi=300) #指定分辨率保存
#        plt.show()
#        
#        plt.plot(range(EPOCH2), hist.history['loss'])
...
#        
#        
#        np.savetxt("BRCA_methylation_mRNA_without_penalty_term_ml_ordCOX_20210315.csv",hist.history['loss'], delimiter=",")
#        
#    #    import pylab
#    #    pylab.plot(hist.history['loss'])
#    #    print([np.exp(-K.get_value(log_var[0]))**0.5 for log_var in model.layers[-1].log_vars])
#        print('lambda1 for Main Loss=',lambda0[499])
#        print('lambda2 for Auxiliary Loss=',lambda1[499])
#        ###########
#        print('weight1 for Main Loss=',std0[499])
#        print('weight2 for Auxiliary Loss=',std1[499])
#        
#        print('uu for data_gene=',uu)
#        print('vv for methylation data=',vv)
        ###############
        idx1=(np.abs(uu)>1e-7)[:,-1]
#        print('idx1 for data_gene=',idx1)
        W_u=np.arange(input_dim_mRNA)
        W1=W_u[idx1]
        W_uu.extend(W_u[idx1])
        
        idx2=(np.abs(vv)>1e-7)[:,-1]
#        print('idx2 for data_gene=',idx2)
   ...
   ##########################################################################################
    ...
    Gr_index=Au>=7
    Gm_index=Av>=5
    data_mRNA_train3=data_mRNA_train[:,Gr_index]
    data_methylation_train3=np.asarray(data_methylation_train[:,Gm_index])
    data_mm=np.concatenate([data_mRNA_train3,data_methylation_train3],1)
    
    data_mRNA_test3=data_mRNA_test[:,Gr_index]
    data_methylation_test3=np.asarray(data_methylation_test[:,Gm_index])
