...
#    modelX = Sequential()
    ...
    out_gene=Dense(input_dim_mRNA, bias=False, 
#                 W_constraint=UnitNormWithNonnneg(False),
                 #kernel_regularizer=l1(sparsity[0]),
...
#        W_regularizer=l1(sparsity[1])))( clinic_input)
    out_clinic=Dense(input_dim_methylation, bias=False,
#        W_constraint=UnitNormWithNonnneg(False),
       # kernel_regularizer=l1(sparsity[1]),
        name='aux_output')( clinic_input) #activation='linear',
...
#    out = CustomMultiLossLayer2(nb_outputs=2)([out_gene, out_clinic])
    out = CustomMultiLossLayer3(nb_outputs=2)([out_gene, out_clinic])
...
#    print([np.exp(-K.get_value(log_var[0]))**0.5 for log_var in model.layers[-1].log_vars])
    ...
    predicted_main, predicted_aux = dense1_layer_model.predict([data_mRNA_test,data_methylation_test],verbose=1)
