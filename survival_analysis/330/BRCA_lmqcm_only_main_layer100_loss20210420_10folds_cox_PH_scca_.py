...
##############################
##############################
#kidtx = pd.read_csv('GBMLGG_Surv_data_stage_age.csv',usecols=['MEgreenyellow',	'MEgreen','MEturquoise'	,'MEmagenta','MEbrown',	'MEred',	'MEpink',	'MEblack',	'MEpurple',	'MEblue',	'MEyellow',	'MEgrey','erged_data5_stage','erged_data6_Age','V1','erged_data33'])
#kidtx = pd.read_csv('GBMLGG_Surv_data_methylation_mRNA_lmqcm.csv')
...
kidtx =pd.read_csv('BRCA_data_mm_dscca.csv',header=None)#不把第一行作列属性)
...
dataX1 =np.array(kidtx.drop([0,1], axis = 1))

dataX12 =kidtx.drop([0,1], axis = 1)
dataX11 =kidtx
dataX22 =kidtx
col_name=list(dataX11)
colNameDict = {0:'days',1:'vital_status'}                  #将‘源数据列名’改为‘新列名’
dataX22.rename(columns = colNameDict,inplace=True)

y0=[dataX22['days'],dataX22['vital_status']]
y=np.transpose(np.array((dataX22['days'],dataX22['vital_status'])))
...
#kidtx = pd.read_csv('GBMLGG_data_mm_name_dscca_lmQCM_10-10_420.csv')
#dataX1 =kidtx.drop(["Unnamed: 0","ID","vital_status","days"], axis = 1).values
#
#dataX11 =kidtx.drop(["Unnamed: 0","ID"], axis = 1)
##y = np.transpose(np.round(np.array(kidtx["days"]/30),2), kidtx["vital_status"]) # V1=time; erged_data33=status
#ytime= np.round(np.array(kidtx["days"]/30),0)     #np.transpose(np.array(kidtx["days"]))/30 # only V1=time;
...
#concat = np.zeros([m0,n0])
#concat[:, : 17] = data_methylation
#concat[:, 17:] =2*data_mRNA
#dataX=concat
#dataX=torch.cat((data_mRNA,2*data_methylation),1)
...
X, y = get_x_y(df_raw, attr_labels=['vital_status','days'], pos_label=True)
#X=df_raw.drop(['event','time'], axis = 1)
#y=np.asarray((df_raw["event"],df_raw["time"]).values)
...
print(f'right censored samples: {len(df_raw.query("vital_status == False"))}')
print(f'percentage of right censored samples: {100*len(df_raw.query("vital_status == False"))/len(df_raw):.1f}%')
...
#cox.fit(X_trn, y_trn)
#
#ci_cox = concordance_index_censored(y_val["vital_status"], y_val["days"], cox.predict(X_val))
...
#    x_train, x_test, y_train, y_test= train_test_split(dataX, Y, random_state=42)
#    mRNA_names3=[col_name_mRNA[idx_r2] for idx_r2 in idx_r]
    
#    x_train, x_test, y_train, y_test = X.loc[train_index], X.loc[val_index], y[train_index], y[val_index]
    X_trn, X_val, y_trn, y_val = X.loc[train_index], X.loc[val_index], y[train_index], y[val_index]
#    preprocessor = ColumnTransformer(
#    [('cat-preprocessor', OrdinalEncoder(), cat_cols),
#    ('standard-scaler', StandardScaler(), scaling_cols)],
#    remainder='passthrough', sparse_threshold=0)
    ...
    gbc = make_pipeline(preprocessor, GradientBoostingSurvivalAnalysis(random_state=63))
    
    gbc.fit(X_trn, y_trn)
    
    ci_gbc = concordance_index_censored(y_val["vital_status"], y_val["days"], gbc.predict(X_val))
...
#    
##    =, predicted_aux = dense1_layer_model.predict([x_test,x_test,y_test, ystatus2_test],verbose=1)
#    prediction = model.predict(x_test)
##    prediction =predicted_main+0*predicted_aux
#    
#    c_index2=c_index3( np.asarray(y_test[:,0]),np.asarray(prediction), np.asarray(y_test[:,1]))
#    
#    print( c_index2)
#    
#############################################################################################################################    
...
np.savetxt("GBMLGG_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv", ypred, delimiter=",")
np.savetxt("GBMLGG_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv", time_new, delimiter=",")
np.savetxt("GBMLGG_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv", status_new, delimiter=",")
np.savetxt("GBMLGG_ypred_train_median1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv", ypred_train, delimiter=",")

df = pd.read_csv("GBMLGG_prediction1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv",header=None)    
month=np.asarray(pd.read_csv("GBMLGG_ytime_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv",header=None)) 
status=np.asarray(pd.read_csv("GBMLGG_ystatus_test1204_18lstm2222_epoch400_drop01_resnet_without penalty term_only_mainloss316.csv",header=None)) 
...
print("GBMLGG_methylation_mRNA_lmqcm_lr_backend2233_only_main_loss20210316.py") 
print("GBMLGG only_main:dataX=data_rna+methylation:",c_indices_only_main)
