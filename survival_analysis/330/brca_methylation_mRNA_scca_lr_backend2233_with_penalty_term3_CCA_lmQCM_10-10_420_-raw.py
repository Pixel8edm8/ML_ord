...
kidtx = pd.read_csv('brca_Surv_data_methylation_mRNA_all_lmqcm.csv')
dataX1 =kidtx.drop(["Unnamed: 0","ID","V2.x","V3.x"], axis = 1).values
dataX01 =kidtx.drop(["Unnamed: 0","ID","V2.x","V3.x"], axis = 1)
y = np.transpose(np.array((kidtx["V2.x"], kidtx["V3.x"]))) # V1=time; erged_data33=status
...
col_name_meth=col_name[0:17]
col_name_mRNA=col_name[17:n0]
...
data_y_mm_name55=pd.concat([kidtx["ID"],y_data_name,data_mm_name4],1) ############ add colomn name
...
np.savetxt("BRCA_data_mm_dscca_lmqcm_10-10_420.csv",data_mm2, delimiter=",")

#pd.dataFrame.to_save()
data_y_mm_name55.to_csv("BRCA_data_mm_name_dscca_lmqcm_10-10_420.csv")
