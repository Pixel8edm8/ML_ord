...
kidtx =pd.read_csv('BRCA_data_mm_name_dscca_lmqcm_10-10_420.csv')#不把第一行作列属性)
...
#y = np.transpose(np.round(np.array(kidtx["V2.x"]/30),2), kidtx["V3.x"]) # V1=time; erged_data33=status
ytime= np.array(kidtx["V2.x"])     #np.transpose(np.array(kidtx["V2.x"]))/30 # only V1=time;
...
#dataX1 =kidtx.drop(["Unnamed: 0","ID","V3.x","V2.x"], axis = 1).values
#
#dataX11 =kidtx.drop(["Unnamed: 0","ID"], axis = 1)
##y = np.transpose(np.round(np.array(kidtx["V2.x"]/30),2), kidtx["V3.x"]) # V1=time; erged_data33=status
#ytime= np.round(np.array(kidtx["V2.x"]/30),0)     #np.transpose(np.array(kidtx["V2.x"]))/30 # only V1=time;
#ystatus= np.transpose(np.array(kidtx["V3.x"])) #only erged_data33=status
#y=np.transpose([ytime,ystatus])
#Y=np.transpose([ytime,ystatus])
df_raw=dataX11
...
X, y = get_x_y(df_raw, attr_labels=['V3.x','V2.x'], pos_label=True)
...
#print(f'right censored samples: {len(df_raw.query("V3.x== False"))}')
#print(f'percentage of right censored samples: {100*len(df_raw.query("V3.x== False"))/len(df_raw):.1f}%')
...
#cox.fit(X_trn, y_trn)
#
#ci_cox = concordance_index_censored(y_val["V3.x"], y_val["V2.x"], cox.predict(X_val))
...
#    remainder='passthrough', sparse_threshold=0)
    ...
    ci_gbc = concordance_index_censored(y_val["V3.x"], y_val["V2.x"], gbc.predict(X_val))
